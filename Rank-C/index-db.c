...
int main (void)
{
  ...
  if((fnme=fopen("Rank-C/index-db/data.nme","w"))==NULL)
  {
    printf("Cannot create data.nme file...\n");
    return 0;
  }
  if((finf=fopen("Rank-C/index-db/data.wdl","w"))==NULL)
  {
    printf("Cannot create file info without doclen [data.wdl]...\n");
    return 0;
  }
  if((finv=fopen("Rank-C/index-db/data.inv","wb"))==NULL)
  {
    printf("Cannot create inverted file [data.inv]...\n");
    return 0;
  }
  if((fpar=fopen("Rank-C/index-db/data.par","wb"))==NULL)
  {
    printf("Cannot create parameter file [data.par]...\n");
    return 0;
  }
  if((fvoc=fopen("Rank-C/index-db/data.voc","w"))==NULL)
  {
        ...
        else
        {
          ...
          while(!feof(fdat))
          {
            ...
            if(isalpha(word[0]))
            {
              ...
              {
                jumlahKata += setData( hashTable,word,docno);
              }
            }
            doclen+=strlen(word);
          }
  ...
  /* do the hash traversal to read all data in the Hash
     and store them in three separate files */
  printf("\nCreating inverted files...\n");
  hashTraversal(hashTable, jumlahKata, finv, fvoc, fpar);
  fclose(fvoc);
  fclose(fpar);
  fclose(finv);
