...
int main(int argc, char *argv[])
{
  ...
  if((finv=fopen("Rank-C/index-db/data.inv","rb"))==NULL)
  {
    printf("Cannot open inverted file [data.inv]...\n");
    return 0;
  }

  if((fpar=fopen("Rank-C/index-db/data.par","rb"))==NULL)
  {
    printf("Cannot open parameter file [data.par]...\n");
    return 0;
  }

  if((fvoc=fopen("Rank-C/index-db/data.voc","r"))==NULL)
  {
    printf("Cannot open vocabulary file [data.voc]...\n");
    return 0;
  }
  if((finf=fopen("Rank-C/index-db/data.wdl","r"))==NULL)
  {
    printf("Cannot open information file [data.wdl]...\n");
    return 0;
  }

  if((fnme=fopen("Rank-C/index-db/data.nme","r"))==NULL)
  {
  ...
  for(i=0; fscanf(finf,"%ld %ld %ld\n",&docno,&doclen,&offset)==3;i++)
  {
...
void query(FILE *finv, FILE *fnme, Term * arrterm, char *query, int N, int HEAPSIZE)
{
  ...
  if(*query!='\0')
  {
    ...
    else
    {
      ...
      for(i=0;i<N;i++)
      {
        accumulator[i]+=accumulator[i]/doclog; /*using L(D)*/
      }
